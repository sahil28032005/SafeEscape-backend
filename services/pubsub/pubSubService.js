        ...
        try {
          ...
          try {
            ...
            const cleanedData = rawData
              ...
              .replace(/:\s*(\w+)([,}])/g, ':"$1"$2');    // Add quotes to string values
            
            console.log('Attempting to parse cleaned data:', cleanedData);
            data = JSON.parse(cleanedData);
          }
          
          // Validate the expected data structure
          if (!data.warning || !data.warning.type) {
            throw new Error('Invalid message format: missing warning type');
          }
          ...
          // Acknowledge the message to prevent infinite retry
          message.ack();
        }
      });
